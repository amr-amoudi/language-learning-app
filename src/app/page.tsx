...
export default async function mainPage() {
    ...
    const decks = userId ? await getDecksForUser(userId) : null;

    return (
        <div
            className="text-app_yellow grid-cols-3 grid-rows-3 grid w-screen h-screen overflow-hidden px-4 py-5 m-0 md:max-w-[1500] md:mx-auto">
            {
                decks
                &&
                <>
                    ...
                    <div className="col-end-3 col-start-2 row-end-3 row-start-2 flex justify-center items-center">
                        <Link href={'/start/' + (decks[0]?.id || "")}
                              className="cursor-pointer text-xl font-medium bg-app_orange text-app_red-dark px-5 py-3 border-2 rounded-lg hover:bg-app_red-light border-app_yellow hover:text-app_yellow">
                            Start!
                        </Link>
                    </div>
                </>
